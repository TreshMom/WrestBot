...
def get_back_keyboard():
    return ReplyKeyboardMarkup(
        [[KeyboardButton(BACK_BUTTON)]], resize_keyboard=True, one_time_keyboard=True
    )
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Вы вошли как администратор", reply_markup=get_main_keyboard_admin()
    )
...
async def handle_change_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введите новую цену подписки (в рублях):", reply_markup=get_back_keyboard()
    )
    return CHANGE_PRICE
...
async def save_new_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.isdigit():
        await update.message.reply_text(
            "Введите число.", reply_markup=get_back_keyboard()
        )
        return CHANGE_PRICE
    ...
    await update.message.reply_text(
        "Цена обновлена.", reply_markup=get_main_keyboard_admin()
    )
    return ConversationHandler.END
...
async def handle_change_period(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введите новый период подписки (в месяцах):", reply_markup=get_back_keyboard()
    )
...
async def save_new_period(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Период подписки обновлён.", reply_markup=get_main_keyboard_admin()
    )
    return ConversationHandler.END
...
async def handle_change_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введите новое описание тренера:", reply_markup=get_back_keyboard()
    )
    return CHANGE_INFO
...
async def save_new_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Описание тренера обновлено.", reply_markup=get_main_keyboard_admin()
    )
    return ConversationHandler.END
...
async def handle_change_contacts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введите новые контактные данные:", reply_markup=get_back_keyboard()
    )
...
async def save_new_contacts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Контактные данные обновлены.", reply_markup=get_main_keyboard_admin()
    )
    return ConversationHandler.END
