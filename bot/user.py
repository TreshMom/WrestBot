...
async def handle_payment_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"💳 Оплатить {price} руб", callback_data="init_payment")],
...
async def handle_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.message.reply_text("Вы вернулись в главное меню", reply_markup=get_main_keyboard_user())
...
async def check_and_remove_expired_subscriptions(context):
    ...
    for user_id, date_end in rows:
        ...
        if now > end_dt:
            ...
            c.execute("UPDATE users SET is_paid = 0, payment_date = NULL, data_end = NULL WHERE user_id = ?", (user_id,))
            conn.commit()
    conn.close()
